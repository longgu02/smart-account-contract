...
async function main() {
	...
	const FastTransferPlugin = await ethers.deployContract("FastTransferPlugin");
	await FastTransferPlugin.waitForDeployment();
	console.log(`const FASTPLUGIN_ADDRESS = "${FastTransferPlugin.target}"`);
}
